...
{
    private readonly IPositionDetailService _service;

    public PositionDetailsController(IPositionDetailService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] PositionDetailRequest request)
    {
        var identityUser = HttpContext.GetIdentityUser();
        var roles = identityUser.Role ?? "";
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] PositionDetailRequest request)
    {
